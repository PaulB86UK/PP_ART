...
```
So this strategy sacrifies an 8% of the data. As this time is a Regression problem I used the R2 score, to check how it performs. The result I obtained *with a little fine tuning* is the following:
```python
    learning_rate  max_depth  n_estimators     Score(R2)
7           0.001          4           500  0.434980
```
